...
public class TaxonDao {
  ...
  public List<Taxon> findAll() {
    ...
    // Aucun paramètre : Connection, PreparedStatement et ResultSet dans un seul try-with-resources
    // (tous fermés automatiquement à la sortie du bloc).
    try (Connection connexion = source.getConnection();
        PreparedStatement ps = connexion.prepareStatement(sql);
        ResultSet rs = ps.executeQuery()) {
      // Plusieurs lignes possibles : on parcourt tout le ResultSet.
      while (rs.next()) {
        taxons.add(depuis(rs));
      }
    } catch (SQLException e) {
      // On traduit l'erreur technique JDBC en exception métier, sans la faire fuiter.
      throw new DataAccessException("Échec de la lecture des taxons", e);
    }
  ...
  public Optional<Taxon> getByCode(String code) {
    ...
    try (Connection connexion = source.getConnection();
        PreparedStatement ps = connexion.prepareStatement(sql)) {
      // On lie le paramètre ? AVANT d'exécuter : valeur transmise à part = pas d'injection SQL.
      ps.setString(1, code);
      // ResultSet dans un try imbriqué car il faut un appel (setString) entre prepare et execute.
      try (ResultSet rs = ps.executeQuery()) {
        // Recherche par clé : au plus une ligne, donc un if (pas un while).
        if (rs.next()) {
          resultat = Optional.of(depuis(rs));
        }
      }
    } catch (SQLException e) {
      throw new DataAccessException("Échec de la lecture du taxon " + code, e);
    }
